...
void ZeroActor::afterNNEvaluation(const std::shared_ptr<NetworkOutput>& network_output)
{
    ...
    if (alphazero_network_) {
        ...
        if (!env_transition.isTerminal()) {
            ...
            getMCTS()->backup(node_path, alphazero_output->win_condition_, alphazero_output->value_, env_transition.getReward());
        } else {
            getMCTS()->backup(node_path, env_transition.getWinCondition(), env_transition.getEvalScore(), env_transition.getReward());
        }
    } else {
        assert(false);
    }
